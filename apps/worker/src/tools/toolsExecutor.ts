...
import { withCircuitBreak, isCircuitOpen } from '../circuitBreaker.js';
 ...
 * We build parameterized LIKE queries internally.  SQLite LIKE is case-insensitive for ASCII
 * (a-z/A-Z), which covers most WhatsApp name/content data.  For Unicode-case
 * sensitivity a COLLATE NOCASE could be added per-field if needed.
 */
function executeDatasetTool(call: ToolCall): ToolResult {
  ...
  if (queryParam && searchableFields.length > 0) {
    const likeConditions = searchableFields.map(f => `${f} LIKE ?`);
