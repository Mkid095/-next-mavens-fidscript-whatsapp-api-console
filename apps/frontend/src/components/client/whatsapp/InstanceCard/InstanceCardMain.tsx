...
import {
  QrCode, Link2Off, RefreshCw, Wifi, WifiOff, Trash2, AlertTriangle,
  X, Settings2, MoreHorizontal, Copy,
} from 'lucide-react';
import type { Instance } from '../../../../services/api';
import type { InstanceCardProps } from './types';
import InstanceCardStats from './InstanceCardStats';

interface InstanceCardMainProps extends InstanceCardProps {
  confirming: boolean;
  menuOpen: boolean;
  copied: boolean;
  menuRef: React.RefObject<HTMLDivElement | null>;
  onSetConfirming: (v: boolean) => void;
  onSetMenuOpen: (v: boolean | ((o: boolean) => boolean)) => void;
  onSetCopied: (v: boolean) => void;
  onCopyName: () => void;
  onConfirmDelete: () => void;
}

export default function InstanceCardMain({
  inst,
  onConnect,
  onDisconnect,
  onDelete,
  ...
  return (
    <>
          ...
          <div className="relative" ref={menuRef}>
            ...
            <AnimatePresence>
              {menuOpen && (
                ...
                >
                  <button
                    onClick={() => { onSetMenuOpen(false); onSettings(inst); }}
                  ...
                  {isConnected && (
                    <>
                      ...
                      <button
                        onClick={() => { onSetMenuOpen(false); onSyncGroups?.(inst); }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-[11px] text-[#a8a99e] hover:bg-[#2d2813] transition-colors"
                      >
                        <RefreshCw className="w-3.5 h-3.5 text-[#6e684a]" /> Sync Groups
                      </button>
                      <button
                        onClick={() => { onSetMenuOpen(false); onDisconnect(inst); }}
      ...
      {/* Instance stats row */}
      {isConnected && <InstanceCardStats inst={inst} />}
