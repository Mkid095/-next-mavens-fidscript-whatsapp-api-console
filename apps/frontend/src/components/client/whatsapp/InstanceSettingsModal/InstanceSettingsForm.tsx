...
interface InstanceSettingsFormProps {
  inst: InstanceSettingsModalProps['inst'];
  onClose: () => void;
}

export function InstanceSettingsForm({ inst }: InstanceSettingsFormProps) {
  ...
  const unsavedCount = SETTINGS.filter(s => settings[s.key] !== (s.default ?? false)).length;
    ...
    <>
      ...
      <div className="px-5 py-4 border-t border-[#2d2813] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Close button passed via onClose prop in shell */}
          <button
            onClick={onClose}
