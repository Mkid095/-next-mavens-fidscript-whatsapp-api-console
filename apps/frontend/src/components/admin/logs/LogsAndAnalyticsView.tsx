...
export default function LogsAndAnalyticsView({ logs, analytics, onAddLog }: LogsAndAnalyticsViewProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-[#e1e9e5] pb-4 gap-4">
        <div>
          ...
          <p className="text-xs text-graphite mt-1">
            API request logs and message volume analytics across all clients.
          </p>
        </div>
      ...
      {activeTab === 'stream' ? (
        ...
        <TelemetryChart dailyTrends={analytics?.daily_trends} />
      )}
    </div>
  );
}
