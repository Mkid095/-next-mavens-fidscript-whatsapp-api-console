...
import { CheckCircle, XCircle, Globe, Shield, Key, Clock, Monitor, Smartphone } from 'lucide-react';
...
export function StatusBadge({ eventType }: { eventType: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${
        ...
        {isSystem ? <Shield className="w-3 h-3" /> : isApiKey ? <Key className="w-3 h-3" /> :
          <span className="text-[8px] font-bold uppercase">{actorLabel(event).slice(0, 1)}</span>}
      </div>
...
interface AuditRowProps { event: AuditEvent }
...
export function AuditRow({ event }: AuditRowProps) {
  ...
  return (
    <div className={`flex items-start gap-4 px-4 py-3 hover:bg-stone-50/60 transition-colors border-b border-stone-100 ${failed ? 'bg-rose-50/20' : ''}`}>
      <div className="shrink-0 mt-0.5">
        {failed ? <XCircle className="w-4 h-4 text-rose-500" /> : <CheckCircle className="w-4 h-4 text-emerald-500" />}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[10px] font-mono font-semibold text-emerald-800 leading-tight">{event.type}</div>
        {event.resourceType && (
      ...
      <div className="w-36 shrink-0 text-right">
        ...
        <div className="text-[8px] text-stone-300 mt-0.5">{new Date(event.timestamp).toLocaleTimeString()}</div>
      </div>
