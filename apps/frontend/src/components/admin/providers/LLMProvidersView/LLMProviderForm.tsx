/**
 * LLMProviderForm — Add/Edit provider modal.
 */
import { useState } from 'react';
import {
  X, Bot, Key, Loader2, AlertCircle,
} from 'lucide-react';
import { fetchApi } from '../../../../data/api/client.js';
import { LLMProvider, ProviderFormData, PROVIDER_META } from './types';
...
export function LLMProviderForm({
    ...
    try {
      ...
      if (res.success && res.data) {
        const saved = isEdit
          ...
          : await fetchApi<LLMProvider>(
              `/api/admin/llm-providers/${'id' in res.data ? res.data.id : ''}`
            ).then((r) => r.data);
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center bg-stone-900/40 p-4 pt-16"
      onClick={onClose}
    >
      <div
