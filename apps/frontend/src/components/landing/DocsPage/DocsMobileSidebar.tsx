...
interface DocsMobileSidebarProps {
  ...
  onSelectEndpoint: (ep: typeof DOC_GROUPS[0]['endpoints'][0]) => void;
}

export function DocsMobileSidebar({
  open, onClose, activeTab, setActiveTab,
  activeSection, setActiveSection, onSelectGuide, onSelectEndpoint,
                ...
                <div className="flex bg-[#1a1910] rounded-xl p-1">
                  {(['guides', 'api-reference'] as const).map(tab => (
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-colors ${activeTab === tab ? 'bg-yellow-500 text-stone-950' : 'text-[#8a886a]'}`}
                    >
                ...
                <div>
                  {GUIDES.map(g => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm border-b border-[#1e1c10] ${activeSection === g.id ? 'text-yellow-500 font-semibold' : 'text-[#8a886a]'}`}
                    >
                      {g.label}
                    </button>
                  ))}
                ...
                DOC_GROUPS.map(group => (
                  <div key={group.name}>
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-4 py-2.5 text-xs font-bold border-b border-[#1e1c10] ${activeSection === group.name ? 'text-yellow-500' : 'text-[#6a6c5d]'}`}
                    >
