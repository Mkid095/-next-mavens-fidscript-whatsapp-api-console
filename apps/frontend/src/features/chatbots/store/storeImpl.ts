/**
 * ChatbotBuilderStore — Zustand store implementation.
 *
...
import type { ChatbotBuilderStore } from './storeTypes';
...
export const useChatbotBuilderStore = create<ChatbotBuilderStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    updateGeneral: (data) =>
      set((state) => ({
        draft: { ...state.draft, general: { ...state.draft.general, ...data }, isDirty: true },
      })),

    updateAudience: (data) =>
      set((state) => ({
        draft: { ...state.draft, audience: { ...state.draft.audience, ...data }, isDirty: true },
      })),

    updateAIBrain: (data) =>
      set((state) => ({
        draft: { ...state.draft, aiBrain: { ...state.draft.aiBrain, ...data }, isDirty: true },
      })),

    updateKnowledge: (data) =>
      set((state) => ({
        draft: { ...state.draft, knowledge: { ...state.draft.knowledge, ...data }, isDirty: true },
      })),

    updateDataConnections: (data) =>
      set((state) => ({
        draft: { ...state.draft, dataConnections: { ...state.draft.dataConnections, ...data }, isDirty: true },
      })),

    updateTools: (data) =>
      set((state) => ({
        draft: { ...state.draft, tools: { ...state.draft.tools, ...data }, isDirty: true },
      })),

    updateGroups: (data) =>
      set((state) => ({
        draft: { ...state.draft, groups: { ...state.draft.groups, ...data }, isDirty: true },
      })),

    updateHandoff: (data) =>
      set((state) => ({
        draft: { ...state.draft, handoff: { ...state.draft.handoff, ...data }, isDirty: true },
      })),

    updateTest: (data) =>
      set((state) => ({
        draft: { ...state.draft, test: { ...state.draft.test, ...data }, isDirty: true },
      })),
    ...
    getProgress: () => {
      const { draft } = get();
      return Math.round((draft.completedSteps.length / STEP_ORDER.length) * 100);
    },
...
export function scheduleAutosave(clientToken: string, delayMs = 2000) {
  ...
  autosaveTimer = setTimeout(async () => {
    ...
    try {
      localStorage.setItem(`chatbot_draft_${draft.id ?? 'new'}`, JSON.stringify(draft));
    } catch (_) { /* ignore */ }
...
export function restoreDraft(botId?: string): ChatbotDraft | null {
  try {
    ...
    if (stored) return JSON.parse(stored) as ChatbotDraft;
  } catch (_) { /* ignore */ }
  return null;
}
