...
import { Search, Loader2, Users2, Check } from 'lucide-react';
...
export function GroupPicker({ selectedIds, onToggle }: GroupPickerProps) {
  ...
  const loadGroups = useCallback(async () => {
    setLoading(true);
    try {
      const res = await import('../../../../data/api/client').then(m => m.fetchApi<ContactGroup[]>('/api/groups'));
  ...
  return (
    <div className="space-y-3 bg-[#0d0c0a] border border-[#2d2813] rounded-xl p-4">
      <div className="flex items-center justify-between">
        <p className="text-xs font-bold text-yellow-400">Select Groups</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-6">
          <Users2 size={24} className="mx-auto text-[#3d3823] mb-2" />
          <p className="text-xs text-[#6e684a]">No groups found</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-[#6e684a]">{g.member_count} members</p>
                </div>
              </button>
            );
          })}
