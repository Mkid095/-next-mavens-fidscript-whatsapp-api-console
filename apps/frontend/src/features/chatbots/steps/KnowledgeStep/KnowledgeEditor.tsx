/**
 * KnowledgeEditor — add/edit modal for a knowledge source.
 */
...
const SOURCE_TYPES: {
  ...
  { value: 'url',      label: 'Website URL',    description: 'Scrape content from a webpage',          icon: Globe },
  { value: 'faq',      label: 'FAQ',             description: 'Structured question & answer pairs',      icon: FileText },
  { value: 'text',     label: 'Plain Text',     description: 'Paste or write free-form text',          icon: FileText },
  { value: 'json',     label: 'JSON',            description: 'Structured data (products, inventory)', icon: Database },
  { value: 'pdf',      label: 'PDF',             description: 'Upload a PDF document',                  icon: FileText },
  { value: 'csv',      label: 'CSV',             description: 'Spreadsheet or table data',              icon: FileSpreadsheet },
  { value: 'database', label: 'Database',       description: 'Connect to your database',              icon: Server },
  { value: 'api',      label: 'API Endpoint',   description: 'Fetch data from an external API',       icon: Code },
];
...
export function KnowledgeEditor({
  ...
  const handleAdd = () => {
    if (!name.trim()) return;
    onAdd({ type, name: name.trim(), status: 'indexing', chunkCount: 0, ref: ref.trim(), content: content.trim(), errorMessage: undefined });
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1a1915] border border-[#2d2813] rounded-2xl p-6 max-w-lg w-full space-y-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[#6e684a] hover:text-white transition"><X className="w-5 h-5" /></button>
        </div>
            ...
            {visibleTypes.map(t => (
              <button
                ...
                className={`flex items-center gap-2.5 p-3 rounded-xl border text-left transition ${
                  type === t.value ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-[#0d0c0a] border-[#2d2813] hover:border-[#3d3823]'
                }`}
              >
                <span className={type === t.value ? 'text-yellow-400' : 'text-[#6e684a]'}>
                  {(() => { const Icon = t.icon; return <Icon size={16} />; })()}
                </span>
                <span className={`text-xs font-semibold ${type === t.value ? 'text-white' : 'text-[#a8a99e]'}`}>{t.label}</span>
              </button>
            ))}
          </div>
          {!showAll && SOURCE_TYPES.length > 6 && (
            <button onClick={() => setShowAll(true)} className="mt-2 text-xs text-[#6e684a] hover:text-white flex items-center gap-1 transition">
              <ChevronDown className="w-3.5 h-3.5" />{SOURCE_TYPES.length - 6} more types
            </button>
          )}
        </div>
        ...
        {(type === 'url' || type === 'api') && (
          <div>
            ...
            {type === 'url' && <p className="mt-1 text-[10px] text-[#5a554a]">We'll scrape the page content and index it for your chatbot.</p>}
          </div>
        )}
        ...
        {type === 'text' && (
          <div>
            <label className="block text-xs font-bold text-[#8f834a] mb-1.5">Text Content</label>
            <textarea
              value={content}
              onChange={e => setContent(e.target.value)}
              rows={7}
              placeholder="Paste or type the text you want your chatbot to know..."
        ...
        {type === 'json' && (
          <div>
            <label className="block text-xs font-bold text-[#8f834a] mb-1.5">JSON Data</label>
            <textarea
              value={content}
              onChange={e => setContent(e.target.value)}
              rows={7}
              placeholder={'[{"name": "Widget Pro", "price": 29.99}, ...]'}
        ...
        {(type === 'pdf' || type === 'csv') && (
          <div className="border-2 border-dashed border-[#2d2813] rounded-xl p-6 text-center">
            ...
            <p className="text-sm text-white font-semibold">{type === 'pdf' ? 'Upload PDF' : 'Upload CSV'}</p>
            <p className="text-xs text-[#6e684a] mt-1">File upload coming soon — paste your content in the text field below for now</p>
            {type === 'csv' && (
              <textarea
                value={content}
                onChange={e => setContent(e.target.value)}
                rows={5}
                placeholder="name,description,price\nWidget Pro,Our best widget,29.99"
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className="flex-1 py-3 bg-[#2d2813] hover:bg-[#3d3823] text-white rounded-xl text-sm font-semibold transition">Cancel</button>
          <button
            onClick={handleAdd}
            disabled={!name.trim()}
