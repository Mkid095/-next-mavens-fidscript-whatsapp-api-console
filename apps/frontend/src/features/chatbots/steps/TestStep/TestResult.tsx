/**
 * TestResult — debug sidebar with what-happened breakdown.
 */
...
function DebugRow({
  ...
  const colorClass = status === 'matched' ? 'text-green-400' : status === 'none' ? 'text-[#6e684a]' : 'text-yellow-400';
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-1.5 text-[10px] text-[#6e684a]">{icon}{label}</div>
      <div className="flex items-center gap-1.5 pl-4">
        ...
        <span className={`text-xs ${status === 'none' ? 'text-[#6e684a]' : 'text-white'} truncate`}>{value}</span>
      </div>
  ...
  return (
    <div className="w-64 shrink-0 bg-[#0d0c0a] border border-[#2d2813] rounded-2xl overflow-hidden flex flex-col max-h-[460px]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {debug.knowledge_sources && debug.knowledge_sources.length > 0 && (
          <div className="space-y-1.5">
            <div className="flex items-center gap-1.5 text-[10px] text-[#6e684a]">
              <BookOpen className="w-3.5 h-3.5 text-blue-400" />Knowledge used
            </div>
        ...
        {!debug.matched_trigger && (
          <DebugRow icon={<XCircle className="w-3.5 h-3.5 text-[#6e684a]" />} label="Trigger" value="No trigger matched" status="none" />
        )}
        ...
        {(debug.latency_ms !== undefined || debug.tokens_used !== undefined) && (
          <div className="space-y-1">
            ...
            {debug.latency_ms !== undefined && (
              <div className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-[#6e684a]" /><span className="text-xs text-white">{formatLatency(debug.latency_ms)}</span></div>
            )}
            {debug.tokens_used !== undefined && (
              <div className="flex items-center gap-1.5"><Database className="w-3.5 h-3.5 text-[#6e684a]" /><span className="text-xs text-white">{debug.tokens_used.toLocaleString()} tokens</span></div>
            )}
          </div>
        )}
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-[#2d2813] rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${debug.confidence >= 0.8 ? 'bg-green-400' : debug.confidence >= 0.5 ? 'bg-yellow-400' : 'bg-red-400'}`}
                  style={{ width: `${Math.round(debug.confidence * 100)}%` }}
                />
              </div>
              <span className="text-xs text-white font-mono">{Math.round(debug.confidence * 100)}%</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-3 border-t border-[#2d2813] shrink-0">
        <button onClick={onToggleExpanded} className="w-full flex items-center justify-between text-[10px] text-[#6e684a] hover:text-white transition">
