/**
 * Tool execution implementation — executeTool wrapper + all dispatch helpers.
 */
import db from '../../database.js';
import { loadDataSource, buildAuthHeaders, resolveExecutor, substitutePath, type ExecutorConfig } from './toolExecutionHelpers.js';
...
// ---------------------------------------------------------------------------
// executeTool — public entry point (wraps executeToolImpl)
// ---------------------------------------------------------------------------

export async function executeTool(input: ToolRunnerInput): Promise<ToolRunResult> {
  return executeToolImpl(input);
}
...
async function executeToolImpl(input: ToolRunnerInput): Promise<ToolRunResult> {
