/**
 * chatbotEngineImpl.ts — core engine: bot lookup + trigger evaluation + rule engine.
 *
 * Public exports:
 *   evaluateTriggers()
 *   findBotsForMessage()
...
export function evaluateTriggers(
  ...
  if (!bot) {
    return {
      botId,
      trigger: { triggered: false, confidence: 0, requiresPreviousBotReply: false, satisfied: true },
      rule: { matched: false, action: 'skip', actionConfig: {}, conditionsJson: '[]' },
  ...
  if (['CLOSED', 'AGENT'].includes(conversationState) && triggerResult.triggerType !== 'always') {
    return {
      botId,
      trigger: triggerResult,
      rule: { matched: false, action: 'skip', actionConfig: {} },
      conversationState,
      isHandoff: true,
      shouldRespond: false,
      skipReason: conversationState === 'AGENT' ? 'handoff_active' : 'manual_override',
    };
  }
...
function evaluateCondition(cond: Condition, ctx: EvaluationContext): boolean {
  ...
  if (parts[0] === 'contact' && ctx.contactId) {
    const contact = db.prepare('SELECT * FROM contacts WHERE id = ?').get(ctx.contactId) as Record<string, unknown> | undefined;
    ...
    if (parts[1] === 'hour') fieldValue = new Date().getHours();
  }
