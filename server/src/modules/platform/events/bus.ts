...
class InProcessBusImpl implements EventBus {
  ...
  async emit<T extends DomainEventType>(
    ...
    // Per-type subscribers receive the raw payload (back-compat with search
    // indexer, analytics projectors, AI inbound, automations).
    this.emitter.emit(type, payload);

    // Wildcard subscribers receive an envelope so they can route by type.
    // workspaceId/actorUserId are taken from meta first, then from the
    // enriched payload that dispatch.ts stamps on.
