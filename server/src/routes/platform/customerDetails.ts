...
// + write audit_logs rows per spec §6.4.
// =============================================================================
...
router.get('/:id/tags', (req: Request, res: Response) => {
  try {
    ...
    const rows = db.prepare('SELECT id, tag, created_at FROM customer_tags WHERE customer_id = ? ORDER BY created_at DESC')
      .all(req.params.id);
...
router.post('/:id/tags', (req: Request, res: Response) => {
  try {
    ...
    const existing = db.prepare('SELECT id FROM customer_tags WHERE customer_id = ? AND tag = ?')
      .get(req.params.id, tag);
    if (existing) { res.json({ success: true, data: { id: (existing as { id: string }).id, tag, dedup: true } }); return; }
    const id = uuidv4();
    db.prepare('INSERT INTO customer_tags (id, customer_id, tag) VALUES (?, ?, ?)').run(id, req.params.id, tag);
...
router.delete('/:id/tags/:tag', (req: Request, res: Response) => {
  try {
    if (!ownedCustomer(req)) { res.status(404).json({ success: false, error: 'Not found' }); return; }
    db.prepare('DELETE FROM customer_tags WHERE customer_id = ? AND tag = ?').run(req.params.id, req.params.tag);
...
router.post('/:id/notes', (req: Request, res: Response) => {
  try {
    ...
    const id = uuidv4();
    db.prepare('INSERT INTO customer_notes (id, customer_id, author_user_id, body) VALUES (?, ?, ?, ?)')
      .run(id, req.params.id, wsId(req), body);
...
router.put('/:id/assignment', (req: Request, res: Response) => {
  try {
    ...
    const existing = db.prepare('SELECT id FROM customer_assignments WHERE customer_id = ?').get(req.params.id) as { id: string } | undefined;
    if (existing) {
      db.prepare('UPDATE customer_assignments SET owner_user_id = ?, team_id = ? WHERE id = ?')
        .run(ownerUserId, teamId, existing.id);
    } else {
      db.prepare('INSERT INTO customer_assignments (id, customer_id, owner_user_id, team_id) VALUES (?, ?, ?, ?)')
        .run(uuidv4(), req.params.id, ownerUserId, teamId);
    }
