...
import { useFlow } from '../../../data/hooks/automation/useFlows.js';
...
export default function AutomationEditorMain({ flowId }: { flowId: string }) {
      ...
      <ol className="mb-2 space-y-1.5">
        {nodes.map((n, idx) => (
          <li key={n.id ?? idx} className="rounded-lg border border-stone-200 bg-stone-50 p-2 text-xs">
            ...
            <ConditionBuilder node={n} onChange={(patch) => updateNode(idx, patch)} />
          </li>
        ))}
      </ol>
