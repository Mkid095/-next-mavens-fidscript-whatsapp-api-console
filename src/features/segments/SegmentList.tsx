...
interface SegmentListProps {
  ...
  /** When onPick is provided, a "Use in campaign" button appears; otherwise the
   *  list is rendered in management mode. */
  pickMode?: boolean;
}
...
export default function SegmentList({ onCreate, onPick, pickMode }: SegmentListProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex-1 min-w-[160px] relative">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search segments…"
            className="w-full pl-7 pr-2 py-1.5 text-xs border border-[#eaebe4] bg-white rounded-lg" />
        </div>
        {!pickMode && (
          <button onClick={onCreate}
            className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-bold bg-forest-deep text-white rounded-lg">
      ...
      {loading && segments.length === 0 && <p className="text-xs text-stone-400">Loading segments…</p>}
      ...
      {filtered.length === 0 && !loading ? (
        <div className="p-8 border-2 border-dashed border-stone-200 rounded-2xl text-center">
          ...
          <p className="text-xs text-stone-500">{pickMode ? 'No segments yet — create one to target by audience rules.' : 'No segments yet. Build one to target campaigns by audience rules.'}</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(s => (
            <div key={s.id} className="flex items-center gap-3 p-3 bg-white border border-[#eaebe4] rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-stone-700 truncate">{s.name}</p>
              ...
              {pickMode && onPick && (
                <button onClick={() => onPick(s)}
                  className="px-2.5 py-1 text-[10px] font-bold bg-forest-deep text-white rounded-lg shrink-0">
                  Use
                </button>
              )}
              {!pickMode && (
                <button onClick={() => { if (confirm(`Delete segment "${s.name}"?`)) remove(s.id); }}
                  className="p-1.5 text-stone-400 hover:text-red-600 shrink-0">
