  ...
  return (
    <div className="p-3 bg-[#f9f9f2] border border-[#eaebe4] rounded-xl space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-[10px] font-bold text-graphite uppercase">Preview</p>
        <button onClick={onRun} disabled={loading}
          className="px-2.5 py-1 text-[10px] font-bold bg-forest-deep text-white rounded-lg disabled:opacity-50">
      ...
      {preview && (
        <div className="space-y-1.5">
          <p className="text-sm font-bold text-forest-deep">{preview.customer_count} customer{preview.customer_count === 1 ? '' : 's'} match</p>
          ...
          <p className="text-[9px] text-stone-400">Computed {new Date(preview.computed_at).toLocaleString()}</p>
        </div>
      )}
    </div>
  );
