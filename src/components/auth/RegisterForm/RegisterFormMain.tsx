...
interface RegisterFormMainProps {
...
export function RegisterFormMain({ onSuccess, onError, onLoadingChange }: RegisterFormMainProps) {
  ...
  const sendCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !email.trim() || !phone.trim()) { onError('Please fill in all fields.'); return; }
    setBusy(true);
    onError('');
    const res = await authApi.clientRequestCode(name.trim(), email.trim().toLowerCase(), phone.trim());
    setBusy(false);
    if (res.success) { setStep('code'); start(60); setCode(''); }
    else { onError(res.error || 'Failed to send code. Please try again.'); }
  };

  const verify = async (e: React.FormEvent) => {
    e.preventDefault();
    if (code.replace(/\s/g, '').length < 6) { onError('Please enter the 6-digit code.'); return; }
    setBusy(true);
    onError('');
    ...
    } else { onError(res.error || 'Invalid or expired code.'); }
  };
  ...
  const resend = async () => {
    ...
    const res = await authApi.clientRequestCode(name.trim(), email.trim().toLowerCase(), phone.trim());
    setBusy(false);
    if (res.success) { start(60); setCode(''); }
    else { onError(res.error || 'Failed to resend code.'); }
  };

  if (step === 'code') {
    return (
      <form onSubmit={verify} className="space-y-4">
        <button type="button" onClick={() => { setStep('details'); onError(''); }}
        ...
        <div className="flex justify-between items-center text-[10px]">
          ...
          <button type="button" onClick={resend} disabled={!canResend || isSubmitting}
  ...
  return (
    <form onSubmit={sendCode} className="space-y-4">
      <div>
        ...
        <input type="text" required disabled={isSubmitting} placeholder="Jane Wanjiku"
          value={name} onChange={(e) => setName(e.target.value)} className={INPUT} />
      </div>
      <div>
        <label className={LABEL}>Business Email</label>
        <input type="email" required disabled={isSubmitting} placeholder="jane@company.co.ke"
          value={email} onChange={(e) => setEmail(e.target.value)} className={`${INPUT} font-mono`} />
      </div>
      <div>
        <label className={LABEL}>Phone Number</label>
        <input type="text" required disabled={isSubmitting} placeholder="254712345678"
          value={phone} onChange={(e) => setPhone(e.target.value)} className={`${INPUT} font-mono`} />
      </div>
