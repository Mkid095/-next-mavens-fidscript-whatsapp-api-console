...
export function LoginFormMain({ defaultEmail, onSuccess, onError, onLoadingChange }: LoginFormMainProps) {
  ...
  const sendCode = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!email.trim()) { onError('Please enter your email.'); return; }
    setBusy(true);
    onError('');
  ...
  if (step === 'code') {
    return (
      <form onSubmit={verify} className="space-y-4">
        <button type="button" onClick={() => { setStep('email'); onError(''); }}
  ...
  return (
    <form onSubmit={sendCode} className="space-y-4">
      <div>
        ...
        <input type="email" required disabled={isSubmitting} placeholder="you@company.co.ke"
