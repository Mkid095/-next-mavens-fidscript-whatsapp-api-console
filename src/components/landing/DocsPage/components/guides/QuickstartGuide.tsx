...
export function QuickstartGuide() {
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <Callout type="info">
        <p>
          <strong className="text-white">New to FIDScript?</strong> You'll need an account
      ...
      {[
        ...
        <div key={n} className="flex gap-4 mb-6">
          <div className="w-7 h-7 rounded-full bg-yellow-500 text-stone-950 flex items-center justify-center text-sm font-bold shrink-0 mt-0.5">
            {n}
          </div>
          <div>
            ...
            <ul className="space-y-1.5">
              {items.map(i => (
                <li
                  key={i}
                  className="text-xs text-[#8a886a] pl-3 relative before:content-['•'] before:absolute before:left-0 before:text-[#4a4a3a]"
                >
                  {i}
                </li>
              ))}
