...
interface ClientsViewMainProps {
  clients: Client[];
...
export function ClientsViewMain({
  clients, onAddClient, onToggleClient, onResetKey, onDeleteClient, onAwardTokens,
}: ClientsViewMainProps) {
  ...
  const handleDetailResetKey = async (id: string) => { await onResetKey?.(id); };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#60737a] mt-1">Manage client accounts and their messaging API access.</p>
        </div>
      ...
      <div className="p-4 bg-white border border-[#eaebe4]/80 rounded-2xl shadow-sm">
        <div className="relative">
          ...
          <input type="text" placeholder="Search clients by name or email..." value={searchTerm}
      ...
      <ClientTable clients={filtered} onToggleClient={onToggleClient} onResetKey={onResetKey}
        onDeleteClient={onDeleteClient} onAwardTokens={(cli) => setAwardTarget({ id: cli.id, name: cli.name, email: cli.email })}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          ...
          <button onClick={() => setIsModalOpen(true)} className="mt-4 text-sm text-yellow-600 hover:text-yellow-700 font-semibold">
            Add your first client
          </button>
        </div>
      )}

      <CreateClientModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={onAddClient} />

      <AwardTokensModal isOpen={!!awardTarget} client={awardTarget} onClose={() => setAwardTarget(null)}
        onAwarded={(id, newBalance) => { onAwardTokens?.(id, newBalance); setAwardTarget(null); }} />

      <ClientDetailModal isOpen={!!detailClient} client={detailClient} onClose={() => setDetailClient(null)}
        onToggle={handleDetailToggle} onResetKey={handleDetailResetKey} onAwardTokens={handleAwardFromDetail} />
    </div>
  );
}
