...
interface GoogleOAuthSectionProps {
  googleStatus: { linked: boolean; name?: string; email?: string; picture?: string } | null;
    ...
    <div className="bg-[#181711] border border-[#2d2813] rounded-2xl p-4 space-y-3">
      ...
      {!googleStatus ? (
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onSync} disabled={googleSyncing} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#eab308] text-[#181711] text-[10px] font-bold rounded-xl hover:bg-yellow-400 transition-all disabled:opacity-50 shrink-0">
        ...
        <>
          <button onClick={onLink} disabled={googleLoading} className="w-full flex items-center justify-center gap-2 py-2 bg-[#4285F4] hover:bg-[#3367D6] text-white text-xs font-bold rounded-xl transition-all disabled:opacity-60">
            {googleLoading ? (
